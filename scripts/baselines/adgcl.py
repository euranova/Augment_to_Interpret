...
from augment_to_interpret.competitors.ad_gcl import (
    save_adcgl_models, run_one_epoch, ADGCLClusteringEmbeddings,
    load_adgcl_models_inplace, get_adgcl_models)
from augment_to_interpret.complex_utils.param_functions import deduce_batch_size, get_adgcl_args
...
if __name__ == "__main__":
    # no correction of the batchnorm when running AD-GCL
    ...
    PATH_RESULTS = Path(C.PATH_RESULTS, "adgcl")
    ...
    for seed, dataset_name in itertools.product(SEEDS, DATASETS):
        ...
        evaluator_class = EV_gsat.GSATEvaluator()
        evaluator = EV_gsat.EmbeddingEvaluation(
            base_model=LinearSVC(dual=False, fit_intercept=True),
        ...
        view_optimizer = torch.optim.Adam(view_learner.parameters(),
                                          lr=args["view_lr"])

        ee = EV_gsat.EmbeddingEvaluation(LinearSVC(dual=False, fit_intercept=True),
                                         evaluator_class,
                                         task,
                                         num_tasks,
                                         device,
        ...
        for epoch in range(args["epochs"]):
            all_embs_train, all_clf_labels_train, edge_att_train, gt_edges_train = run_one_epoch(
                loaders['train'], model, model_optimizer, view_learner, batch_size,
                view_optimizer, args["reg_lambda"], device=device, train=True,
            )
            all_embs_val, all_clf_labels_val, edge_att_val, gt_edges_val = run_one_epoch(
                loaders['valid'], model, model_optimizer, view_learner, batch_size,
                view_optimizer, args["reg_lambda"], device=device, train=True,
            )
            all_embs_test, all_clf_labels_test, edge_att_test, gt_edges_test = run_one_epoch(
                loaders['test'], model, model_optimizer, view_learner, batch_size,
                view_optimizer, args["reg_lambda"], device=device, train=True,
            )
            ...
            if unsup_result_dic['train_sil'] > best_silhouette:
                ...
                save_adcgl_models(model, view_learner, path_best_silhouette_model,
            ...
            if att_auroc_val is not None and att_auroc_val > best_interp_acc:
                ...
                save_adcgl_models(model, view_learner, path_best_interp_model,
                                  final_dic, model_name="best interp model")
            if val_score > best_clf_acc:
                # TODO: in AD-GCL this if and the next are the same. What to do about it?
                best_clf_acc = val_score
                save_adcgl_models(model, view_learner, path_best_clf_model,
            ...
            if downstream_score_valid > best_downstream_clf_acc:
                ...
                save_adcgl_models(model, view_learner, path_best_downstream_clf_model,
                                  final_dic, model_name="best downstream model")
            if epoch + 1 == args["epochs"]:
                save_adcgl_models(model, view_learner, path_final_model,
    ...
    for analyse in ["final_model", "best_downstream_clf_model"]:
        ...
        for dataset_name in DATASETS:
            for seed in SEEDS:
                ...
                model, view_learner = get_adgcl_models(args, x_dim, edge_attr_dim, device)
                load_adgcl_models_inplace(loading_path, model, view_learner, device)
                wrapper = ADGCLClusteringEmbeddings(model.encoder, view_learner)
        ...
        def produce_for_this_dataset(this_dataset, this_seed):
            ...
            model_config = {
                "use_edge_attr": gsat.clf.use_edge_attr, "n_layers": args["num_gc_layers"]}
