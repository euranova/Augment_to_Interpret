""" Encoder for the Zinc dataset. Inspired by AD-GCL code
https://github.com/susheels/adgcl/blob/2605ef8f980934c28d545f2556af5cc6ff48ed18/unsupervised/encoder/zinc_encoder.py
"""
...
class AdgclZincEncoder(torch.nn.Module):
    """ Code highly copy-pasted from AD-GCL
    https://github.com/susheels/adgcl/blob/2605ef8f980934c28d545f2556af5cc6ff48ed18/unsupervised/encoder/zinc_encoder.py
    """

    def __init__(self, num_atom_type, num_bond_type, emb_dim=100, num_gc_layers=5, drop_ratio=0.0,
                 pooling_type="standard", is_infograph=False):
        super(AdgclZincEncoder, self).__init__()
    ...
    def get_embeddings(self, loader, device, is_rand_label=False):
        ...
        with torch.no_grad():
            for data in loader:
                ...
                x, _ = AdgclZincEncoder.forward(self, batch, x, edge_index, edge_attr, edge_weight)
...
class ZincEncoder(AdgclZincEncoder):
    """ Wrapper to make it compatible with our pipeline. """

    def __init__(self, num_atom_type, num_bond_type, emb_dim=100, num_gc_layers=5, drop_ratio=0.0,
                 pooling_type="standard", is_infograph=False):
        super().__init__(num_atom_type, num_bond_type, emb_dim=emb_dim, num_gc_layers=num_gc_layers,
                         drop_ratio=drop_ratio, pooling_type=pooling_type, is_infograph=is_infograph)
