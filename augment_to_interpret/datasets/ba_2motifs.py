...
def add_noise_neighbours(data, num_noise, node_indices=0, binary=False, p=0.5, connectedness='high', c=0.001):
...
class SynGraphDataset(InMemoryDataset):
    ...
    def __init__(self, root, name, transform=None, pre_transform=None, num_noise=None):
        self.name = name.lower()
        super(SynGraphDataset, self).__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
        if num_noise is not None:
            self.data = add_noise_features(self.data, num_noise, binary=False, p=0.5)
